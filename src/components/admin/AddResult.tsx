...
const AddResult = () => {
  ...
  const [anotherGradeName, setAnotherGradeName] = useState("");
  const [anotherGradeTeam, setAnotherGradeTeam] = useState("");
  const [anotherGradePoints, setAnotherGradePoints] = useState("3");
  ...
  const resetForm = () => {
    ...
    setAnotherGradeName("");
    setAnotherGradeTeam("");
    setAnotherGradePoints("3");
  };

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();

    if (!selectedProgram || !firstPlaceName || !firstPlaceTeam || !secondPlaceName || !secondPlaceTeam || !thirdPlaceName || !thirdPlaceTeam) {
    ...
    const resultData = {
      ...
      ...(anotherGradeName && {
        another_grade_name: anotherGradeName,
        another_grade_team: anotherGradeTeam || null,
        another_grade_points: parseInt(anotherGradePoints),
      }),
    };
    ...
    <Card>
      ...
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {/* Another Grade (Optional) */}
          <div className="border border-border rounded-lg p-4 space-y-3">
            <h3 className="font-bold">🏆 Special Grade (Optional)</h3>
            <div>
              <Label>Participant Name</Label>
              <Input
                value={anotherGradeName}
                onChange={(e) => setAnotherGradeName(e.target.value)}
                placeholder="Enter name (optional)"
              />
            </div>
            <div>
              <Label>Team</Label>
              <Select value={anotherGradeTeam} onValueChange={setAnotherGradeTeam}>
                <SelectTrigger>
            ...
            <div>
              ...
              <Input
                type="number"
                value={anotherGradePoints}
                onChange={(e) => setAnotherGradePoints(e.target.value)}
              />
            </div>
          </div>
